...
def _process_select_playlist(provider, context, re_match):
    ...
    items = []
    # create playlist
    items.append((''.join(['[B]', context.localize(provider.LOCAL_MAP['youtube.playlist.create']), '[/B]']), 'playlist.create'))
    ...
    if 'watchLater' in my_playlists:
        ...
        items.append((''.join(['[B]', context.localize(provider.LOCAL_MAP['youtube.watch_later']), '[/B]']), watch_later_playlist_id))
