...
class CalculateN:
    ...
    def get_throttling_plan_gen(raw_code):
        ...
        :returns:
            An iterable of sub-iterables, where the first element of each
            sub-iterable is the index of a function in the 'c' array and its
            other elements are indices to the arguments to be sent to it.
        """
        ...
        # (x, y, z, ...), that is, an iterable of ints.
        for command in (plan_code.strip('c)').replace('[', '')
                        .replace(']', '').replace('(', ',')
                        .replace('c', '').split('),')):
            yield (int(i) for i in command.split(','))
    ...
    def calculate_n(self, mutable_n_list):
        ...
        try:
            ...
            for step_iter in self.get_throttling_plan_gen(self.throttling_function_code):
                curr_func = throttling_array[next(step_iter)]
                if not callable(curr_func):
                    ...
                    return None
                curr_func(*(throttling_array[i] for i in step_iter))
        except:
